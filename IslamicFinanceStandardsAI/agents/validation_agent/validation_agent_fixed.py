...
class ValidationAgent:
    ...
    def _evaluate_criterion(self, criterion: str, context: Dict) -> float:
        ...
        try:
            ...
            # Add RAG context if enabled and criterion is appropriate
            if self.use_rag and criterion in ["SHARIAH_COMPLIANCE", "TECHNICAL_ACCURACY", "CONSISTENCY", "FACTUAL_ACCURACY"]:
                # Create a query based on the criterion and context
                query = f"Islamic finance {criterion.lower().replace('_', ' ')} for {context.get('standard_title', '')} {context.get('enhancement_type', '')} {context.get('original_content', '')}"
                self.logger.info(f"Retrieving RAG context for criterion {criterion} with query: {query}")
                
                # Get relevant context from RAG engine
                rag_context = self.rag_engine.get_retrieval_context(query, k=RAG_CONFIG.get("retrieval_k", 3))
            ...
            # Special handling for FACTUAL_ACCURACY criterion if claim verification is enabled
            if self.use_claim_verification and criterion == "FACTUAL_ACCURACY":
                self.logger.info("Performing claim verification for factual accuracy evaluation")
                
                # Extract the content to verify
                content_to_verify = context.get("enhanced_content", "")
                
                # Verify claims in the content
                verification_results = self.claim_verifier.verify_text(content_to_verify)
                
                # Log verification results
                self.logger.info(f"Claim verification rate: {verification_results['verification_rate']:.2f}")
                
                # Add verification results to the prompt for LLM to consider
                prompt = f"""
                {prompt}
                
                CLAIM VERIFICATION RESULTS:
                Verification rate: {verification_results['verification_rate']:.2f}
                Total claims: {verification_results['claims_total']}
                Verified claims: {verification_results['claims_verified']}
                Unverified claims: {verification_results['unverified_claims']}
                """
